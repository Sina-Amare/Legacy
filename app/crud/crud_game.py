...
def create_game_session(db: Session, *, game_in: GameCreate) -> Game:
    """
    Creates a new game session, setting its initial state based on the
    chosen dynasty's unique historical starting resources.
    """
    dynasty = db.get(Dynasty, game_in.dynasty_id)
    if not dynasty:
        raise ValueError(f"Dynasty with id {game_in.dynasty_id} not found.")

    # Get starting resources from the dynasty's definition, with fallbacks.
...
def process_player_decision(db: Session, *, game: Game, decision_in: DecisionSubmit) -> Game:
    ...
    Supports operations like 'add', 'set', and 'multiply'.
    """
